...
def prop2iters_neighbour_radius(curr_iter: int, iters: int, k: int, memory: dict[str, Any]) -> tuple[float, dict[str, Any]]:
...
def relu_like_neighbour_radius(curr_iter: int, iters: int, k: int, memory: dict[str, Any], alpha: float = 0, beta: float = 0) -> tuple[float, dict[str, Any]]:
    if 'init_radius' not in memory:
        memory['init_radius'] = math.sqrt(2)*k
...
def exp_neighbour_radius(curr_iter: int, iters: int, k: int, memory: dict[str, Any], gamma: float = 1, delta: float = 1) -> tuple[float, Any]:
