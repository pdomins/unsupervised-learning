...
def relu_like_neighbour_radius(curr_iter: int, iters: int, k: int, memory: dict[str, Any], alpha: float, beta: float) -> tuple[float, dict[str, Any]]:
    if 'init_radius' not in memory:
        memory['init_radius'] = math.sqrt(2)*k
...
def exp_neighbour_radius(curr_iter: int, iters: int, k: int, memory: dict[str, Any], gamma: float, delta: float) -> tuple[float, Any]:
